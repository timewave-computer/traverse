...
impl CosmosLayoutCompiler {
    ...
    fn generate_storage_entries(contract: &CosmWasmContract) -> Result<Vec<StorageEntry>, CosmosError> {
        ...
        for (index, storage_var) in analysis.storage_variables.iter().enumerate() {
            ...
            if matches!(storage_var.storage_type, crate::contract::StorageType::Map) {
                let map_entry = StorageEntry {
                    ...
                    offset: 0,
                    type_name: format!("t_mapping_bytes_{}",
    ...
    fn generate_type_info(contract: &CosmWasmContract) -> Result<Vec<TypeInfo>, CosmosError> {
        ...
        for storage_var in &analysis.storage_variables {
            ...
            if matches!(storage_var.storage_type, crate::contract::StorageType::Map) {
                let mapping_type = format!("t_mapping_bytes_{}", 
                    Self::cosmos_to_canonical_type(&storage_var.value_type));
        ...
        let contract = match combined_schema {
            Ok(schema) => {
                ...
                if let Some(obj) = schema.as_object() {
                    ...
                    let msg_type = schema_path.file_name()
                        .and_then(|s| s.to_str())
                        .unwrap_or("");
                    
                    let (instantiate_msg, execute_msg, query_msg) = if msg_type.contains("instantiate") {
            ...
            Err(e) => {
                return Err(TraverseError::Serialization(format!("Invalid JSON schema: {}", e)));
            }
        };
        
        // Generate storage entries and type information
        let storage = Self::generate_storage_entries(&contract)
            .map_err(|e| TraverseError::InvalidLayout(format!("Failed to generate storage: {}", e)))?;
        
        let types = Self::generate_type_info(&contract)
            .map_err(|e| TraverseError::InvalidLayout(format!("Failed to generate types: {}", e)))?;
