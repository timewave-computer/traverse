...
use traverse_core::{KeyResolver, LayoutInfo, StorageEntry, TypeInfo, ZeroSemantics};
...
mod tests {
    ...
    fn test_uniswap_v3_mappings() {
        ...
        assert!(
            result.is_ok(),
            "Failed to resolve tick mapping: {:?}",
            result.err()
        );
        ...
        assert!(
            result.is_ok(),
            "Failed to resolve position mapping: {:?}",
            result.err()
        );
        ...
        assert!(
            result.is_ok(),
            "Failed to resolve observation mapping: {:?}",
            result.err()
        );
    }
    ...
    fn test_compound_v3_nested_mappings() {
        ...
        assert!(
            result.is_ok(),
            "Failed to resolve nested mapping: {:?}",
            result.err()
        );
