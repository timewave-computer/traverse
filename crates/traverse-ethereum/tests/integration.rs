...
fn test_resolve_packed_fields() {
    ...
    match (&owner_path.key, &paused_path.key) {
        (Key::Fixed(owner_key), Key::Fixed(paused_key)) => {
            assert_eq!(
                owner_key, paused_key,
                "Packed fields should have same storage slot"
            );
        }
...
fn test_storage_key_derivation() {
    ...
    assert_eq!(
        path1.layout_commitment, path2.layout_commitment,
        "Layout commitment should be same"
    );
}
...
fn test_resolve_all_paths() {
    ...
    if !paths.is_empty() {
        ...
        for path in &paths {
            assert_eq!(
                path.layout_commitment, first_commitment,
...
fn test_invalid_query_handling() {
    ...
    assert!(
        result.is_err(),
        "Should fail for invalid hex in mapping query"
    );
...
fn test_layout_commitment_consistency() {
    ...
    assert_eq!(
        direct_commitment, path_commitment,
        "Layout commitments should match"
    );
}
...
fn test_performance_baseline() {
    ...
    assert!(
        ...
        "Performance baseline: should complete 100 resolutions in under 1 second"
    );
}
...
fn test_resolve_nested_mapping_queries() {
    ...
    assert!(
        result.is_ok(),
        "Failed to resolve nested mapping query: {:?}",
        result.err()
    );
...
fn test_multi_slot_string_storage() {
    ...
    assert!(
        length_result.is_ok(),
        "Failed to resolve string length query: {:?}",
        length_result.err()
    );
    ...
    match length_path.key {
        Key::Fixed(key_bytes) => {
            ...
            assert_eq!(
                &key_bytes[24..32],
                &expected_slot,
                "Length should be stored at original slot"
            );
            assert_eq!(
                &key_bytes[0..24],
                &[0u8; 24],
    ...
    assert!(
        data_result.is_ok(),
        "Failed to resolve string data query: {:?}",
        data_result.err()
    );
    ...
    match data_path.key {
        Key::Fixed(key_bytes) => {
            ...
            assert_ne!(
                ...
                "Data key should be different from slot"
            );
        }
    ...
    match (&data_path.key, &symbol_path.key) {
        (Key::Fixed(name_key), Key::Fixed(symbol_key)) => {
            assert_ne!(
                name_key, symbol_key,
                "Different strings should have different data keys"
            );
        }
    ...
    assert_eq!(
        data_path.key,
        data_result2.unwrap().key,
        "String data key derivation should be deterministic"
    );

    // Test 5: Test both strings have consistent layout commitment
    assert_eq!(
        length_path.layout_commitment, data_path.layout_commitment,
        "All paths should have same layout commitment"
    );
    assert_eq!(
        data_path.layout_commitment, symbol_path.layout_commitment,
        "All paths should have same layout commitment"
    );
    ...
    println!(
        "  Name length slot: {}",
        hex::encode(match length_path.key {
            Key::Fixed(k) => k,
            _ => [0u8; 32],
        })
    );
    println!(
        "  Name data slot: {}",
        hex::encode(match data_path.key {
            Key::Fixed(k) => k,
            _ => [0u8; 32],
        })
    );
    println!(
        "  Symbol data slot: {}",
        hex::encode(match symbol_path.key {
            Key::Fixed(k) => k,
            _ => [0u8; 32],
        })
    );
