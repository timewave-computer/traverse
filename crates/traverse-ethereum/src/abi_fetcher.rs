...
use traverse_core::{LayoutInfo, StorageEntry, TypeInfo, TraverseError};
...
impl AbiFetcher {
    ...
    pub async fn fetch_abi_from_etherscan(&self, contract_address: &str) -> Result<String, TraverseError> {
        let clean_address = contract_address.strip_prefix("0x").unwrap_or(contract_address);
        ...
        let etherscan_response: EtherscanAbiResponse = serde_json::from_str(&response_text)
            .map_err(|e| TraverseError::Serialization(format!("Failed to parse Etherscan response: {}", e)))?;
        
        if etherscan_response.status != "1" {
            return Err(TraverseError::Network(format!(
                "Etherscan API error: {}", etherscan_response.message
            )));
        }
    ...
    pub fn generate_layout_from_abi(&self, contract_address: &str, abi_json: &str) -> Result<LayoutInfo, TraverseError> {
        ...
        let has_balance_of = abi.iter().any(|item| {
            item.name.as_ref().is_some_and(|name| name == "balanceOf")
        });
        
        let has_total_supply = abi.iter().any(|item| {
            item.name.as_ref().is_some_and(|name| name == "totalSupply")
        });
        
        let has_owner = abi.iter().any(|item| {
            item.name.as_ref().is_some_and(|name| name == "owner")
        });
        
        let has_name = abi.iter().any(|item| {
            item.name.as_ref().is_some_and(|name| name == "name")
        });
        
        let has_symbol = abi.iter().any(|item| {
            item.name.as_ref().is_some_and(|name| name == "symbol")
        });
        
        let has_decimals = abi.iter().any(|item| {
            item.name.as_ref().is_some_and(|name| name == "decimals")
        });
        
        let has_allowance = abi.iter().any(|item| {
            item.name.as_ref().is_some_and(|name| name == "allowance")
        });
        ...
        let has_pause = abi.iter().any(|item| {
            item.name.as_ref().is_some_and(|name| {
                admin_functions.iter().any(|func| name == func)
            })
        });
        ...
        let contract_name = format!("Contract_{}", contract_address.strip_prefix("0x").unwrap_or(contract_address));
    ...
    pub async fn fetch_and_generate_layout(&self, contract_address: &str) -> Result<LayoutInfo, TraverseError> {
        ...
        println!("✅ Generated layout with {} storage entries", layout.storage.len());
        Ok(layout)
    }
} 