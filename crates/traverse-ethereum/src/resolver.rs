...
enum QueryParts {
    ...
    NestedMapping { field_name: String, keys: Vec<Vec<u8>> },
    ...
    fn parse_query(&self, query: &str) -> Result<QueryParts, TraverseError> {
        ...
        if query.contains('.') {
            ...
            if parts.len() != 2 {
                return Err(TraverseError::InvalidQuery(
                    format!("Invalid struct field access: {}. Expected format: struct.field", query)
                ));
            }
            ...
            match field_name.as_str() {
                "length" => return Ok(QueryParts::DynamicLength { field_name: struct_name }),
                "data" => return Ok(QueryParts::DynamicData { field_name: struct_name }),
                _ => return Ok(QueryParts::StructField { struct_name, field_name }),
            }
        }
        ...
        if query.contains('[') && query.contains(']') {
            ...
            if open_brackets != close_brackets {
                return Err(TraverseError::InvalidQuery(
                    format!("Mismatched brackets in query: {}", query)
                ));
            }
            
            // Extract field name (everything before first '[')
            let field_name = query.split('[').next()
                .ok_or_else(|| TraverseError::InvalidQuery(
                    format!("Invalid query format: {}", query)
                ))?
                .trim()
                .to_string();
            ...
            while let Some(start) = current.find('[') {
                let end = current.find(']')
                    .ok_or_else(|| TraverseError::InvalidQuery(
                        format!("Unclosed bracket in query: {}", query)
                    ))?;
            ...
            if keys.is_empty() {
                return Err(TraverseError::InvalidQuery(
                    format!("No keys found in mapping query: {}", query)
                ));
            }
    ...
    fn parse_key(&self, key_str: &str) -> Result<Vec<u8>, TraverseError> {
        ...
        if key_str.starts_with("0x") || key_str.len() == 40 || key_str.len() == 64 {
            ...
            if let Ok(num) = key_str.parse::<u64>() {
                ...
                hex::decode(key_str)
                    .map_err(|e| TraverseError::InvalidQuery(
                        format!("Could not parse key '{}' as hex or number: {}", key_str, e)
                    ))
            }
        }
    }
...
impl KeyResolver for EthereumKeyResolver {
    fn resolve(&self, layout: &LayoutInfo, query: &str) -> Result<StaticKeyPath, TraverseError> {
        ...
        match query_parts {
            QueryParts::Field { field_name } => {
                ...
                let entry = layout.storage.iter()
                    .find(|s| s.label == field_name)
                    .ok_or_else(|| TraverseError::PathResolution(format!("Field not found: {}", field_name)))?;
                ...
                let type_info = layout.types.iter()
                    .find(|t| t.label == entry.type_name)
                    .ok_or_else(|| TraverseError::PathResolution(format!("Type not found: {}", entry.type_name)))?;
                ...
                Ok(StaticKeyPath {
                    ...
                    offset: if entry.offset > 0 { Some(entry.offset) } else { None },
                    field_size,
            ...
            QueryParts::Mapping { field_name, key } => {
                ...
                let entry = layout.storage.iter()
                    .find(|s| s.label == field_name)
                    .ok_or_else(|| TraverseError::PathResolution(format!("Mapping not found: {}", field_name)))?;
            ...
            QueryParts::Array { field_name, index } => {
                ...
                let entry = layout.storage.iter()
                    .find(|s| s.label == field_name)
                    .ok_or_else(|| TraverseError::PathResolution(format!("Array not found: {}", field_name)))?;
            ...
            QueryParts::StructField { struct_name, field_name } => {
                // Find the storage entry
                let entry = layout.storage.iter()
                    .find(|s| s.label == struct_name)
                    .ok_or_else(|| TraverseError::PathResolution(format!("Struct not found: {}", struct_name)))?;
            ...
            QueryParts::DynamicLength { field_name } => {
                ...
                let entry = layout.storage.iter()
                    .find(|s| s.label == field_name)
                    .ok_or_else(|| TraverseError::PathResolution(format!("Dynamic field not found: {}", field_name)))?;
    ...
    fn resolve_all(&self, layout: &LayoutInfo) -> Result<Vec<StaticKeyPath>, TraverseError> {
        ...
        for entry in &layout.storage {
            // For now, just resolve simple fields
            // TODO: Add support for generating all mapping paths with example keys
...
mod tests {
    ...
    fn test_distinguish_array_vs_mapping() {
        ...
        let result = resolver.parse_query("balances[742d35Cc6634C0532925a3b8D97C2e0D8b2D9C]").unwrap();
        assert!(matches!(result, QueryParts::Mapping { .. }));
    }
