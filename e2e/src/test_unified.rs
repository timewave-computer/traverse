...
async fn test_auto_generate(cli: &CliRunner, fixtures: &TestFixtures) -> Result<()> {
    ...
    let output = cli.run_success(&[
        ...
        "--rpc-ethereum", "https://mainnet.infura.io/v3/test",
        "--contract-ethereum", "0xA0b86a33E6Cc3b3c7bC8F1DCCF0e6a8F71c1c0123",
        "--output-dir", eth_output_dir.to_str().unwrap(),
        "--dry-run"
    ])?;
    ...
    FileValidator::validate_directory_structure(&eth_output_dir, &[
    ...
    let output = cli.run_success(&[
        ...
        "--rpc-cosmos", "https://rpc.osmosis.zone:443",
        "--contract-cosmos", "cosmos1contract123",
        "--output-dir", cosmos_output_dir.to_str().unwrap(),
        "--dry-run"
    ])?;
    ...
    // Verify output directory structure
    FileValidator::validate_directory_structure(&cosmos_output_dir, &[
        "layout.json", "resolved_queries.json"
    ])?;
    ...
    let output = cli.run_success(&[
        ...
        "--rpc-ethereum", "https://mainnet.infura.io/v3/test",
        "--contract-ethereum", "0xA0b86a33E6Cc3b3c7bC8F1DCCF0e6a8F71c1c0123",
        "--queries-file", queries_file.to_str().unwrap(),
        "--output-dir", eth_queries_output_dir.to_str().unwrap(),
        "--dry-run"
    ])?;
...
async fn test_batch_generate(cli: &CliRunner, fixtures: &TestFixtures) -> Result<()> {
    ...
    let output = cli.run_success(&[
        ...
        "--parallel", "2",
        "--output-dir", batch_output_dir.to_str().unwrap(),
        "--dry-run"
    ])?;
    ...
    FileValidator::validate_directory_structure(&batch_output_dir, &[
        "summary.json"
    ])?;
    
    // Test performance with parallel processing
    let (_, duration) = PerformanceTester::measure_command_time(cli, &[
        "batch-generate",
        batch_config.to_str().unwrap(),
        "--parallel", "1",
        "--output-dir", batch_output_dir.to_str().unwrap(),
    ...
    if duration > std::time::Duration::from_secs(60) {
        return Err(anyhow::anyhow!("Batch processing took too long: {:?}", duration));
    }
...
async fn test_watch(cli: &CliRunner, fixtures: &TestFixtures) -> Result<()> {
    ...
    let result = std::process::Command::new(fixtures.cli_path())
        .args(&[
            ...
            "--config", watch_config.to_str().unwrap()
        ])
        .spawn();
...
async fn test_chain_detection(cli: &CliRunner, fixtures: &TestFixtures) -> Result<()> {
    ...
    let output = cli.run_success(&[
        "auto-generate",
        erc20_abi.to_str().unwrap(),
        "--output-dir", fixtures.path("outputs/chain_detect_test").to_str().unwrap(),
        "--dry-run"
    ])?;
    ...
    let output = cli.run_success(&[
        "auto-generate",
        cw20_schema.to_str().unwrap(),
        "--output-dir", fixtures.path("outputs/chain_detect_test2").to_str().unwrap(),
        "--dry-run"
    ])?;
    ...
    let output = cli.run_success(&[
        ...
        "--output-dir", fixtures.path("outputs/chain_detect_test3").to_str().unwrap(),
        "--dry-run"
    ])?;
    ...
    let output = cli.run_success(&[
        "auto-generate",
        cw721_schema.to_str().unwrap(),
        "--output-dir", fixtures.path("outputs/chain_detect_test4").to_str().unwrap(),
        "--dry-run"
    ])?;
...
async fn test_error_handling(cli: &CliRunner, fixtures: &TestFixtures) -> Result<()> {
    ...
    let result = cli.run(&[
        ...
        "--output-dir", fixtures.path("outputs/error_test").to_str().unwrap(),
        "--dry-run"
    ]);
    
    if let Ok(output) = result {
        if output.status.success() {
            return Err(anyhow::anyhow!("Expected auto-generate to fail with nonexistent file"));
        }
    ...
    if let Ok(output) = result {
        if output.status.success() {
            return Err(anyhow::anyhow!("Expected batch-generate to fail with nonexistent config"));
        }
    }
    ...
    if let Ok(output) = result {
        if output.status.success() {
            return Err(anyhow::anyhow!("Expected auto-generate to fail without arguments"));
        }
    ...
    if cfg!(unix) {
        let result = cli.run(&[
            "auto-generate",
            fixtures.ethereum_abis.get("erc20").unwrap().to_str().unwrap(),
            "--output-dir", "/root/readonly",
            "--dry-run"
        ]);
