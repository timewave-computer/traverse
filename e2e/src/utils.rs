...
impl CliRunner {
    ...
    #[allow(dead_code)]
    pub fn run_with_exit_code(&self, args: &[&str], expected_code: i32) -> Result<(String, String)> {
        let output = self.run(args)?;
...
impl OutputValidator {
    ...
    pub fn validate_traverse_format(content: &str) -> Result<()> {
        ...
        if !content.contains("storage_key") && !content.contains("slot") {
            return Err(anyhow::anyhow!("Traverse format missing storage key information"));
        }
    ...
    pub fn validate_coprocessor_format(json: &Value) -> Result<()> {
        ...
        if let Some(obj) = json.as_object() {
            ...
            if !obj.keys().any(|k| k.contains("storage") || k.contains("layout") || k.contains("query")) {
                return Err(anyhow::anyhow!("Coprocessor format missing storage-related fields"));
            }
        } else {
...
impl PerformanceTester {
    ...
    pub fn measure_command_time(cli: &CliRunner, args: &[&str]) -> Result<(String, std::time::Duration)> {
